Create a US Core MedicationAdministration Profile

1. Use US Core MedicationRequest  and US Core MedicationDosage as example template StructureDefinitions
...
2. Include only these Mandatory (m = mandatory in base) and Must Support elements: status (m), medication[x] (m), subject (m), effective[x] (m), performer, request, dosageInstruction
3. Bindings are the  same as for US Core MedicationRequest for:
   - MedicationCategory.category:us-core
   - MedicationCategory.medication[x]
   - MedicationCategory.dosageInstruction sub elements

4. Apply voice constraints to `instance-description` and any narrative:
